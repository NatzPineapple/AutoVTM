...
const Seitas = {
  ...
  /* AS DUAS TRAVAS DO LIVRO QUE NINGUÉM COBRAVA.  (§77)

     Básico, pág. 176, sobre o Fazendeiro: *"Os Ventrue não podem
     escolher este tipo de Predador"* e *"Você não pode escolher
     Fazendeiro se sua Potência de Sangue for 3 ou mais"*. E sobre o
     Sacoleiro: *"Os Ventrue não podem escolher este tipo"*.

     Nenhuma das três existia aqui: um Ventrue podia sair do criador
     como Fazendeiro, que é o único clã do livro proibido de sê-lo.

     A Potência de Sangue vem POR PARÂMETRO: quem a calcula é
     `derivados`, da área Ficha, e esta é a camada de dados — alcançar
     para cima é o que o teste de fronteiras barra, e barrou na
     primeira tentativa desta função. Sem o parâmetro, a trava do
     Fazendeiro não é aplicada, e isso fica dito em vez de silenciado. */
  predadorPermitido(x, f, { potencia = null } = {}) {
    const p = this.perfil(f.seita);
    if (x.seita && x.seita !== p.id) return false;
    if (x.seita && p.predadores.extras !== x.seita) return false;
    if (x.claVetado && f.cla && x.claVetado.includes(f.cla)) return false;
    if (x.potenciaMaxima != null && potencia != null && potencia > x.potenciaMaxima) return false;
    return this.visivel(x);
  },

  predadoresPermitidos(f, opcoes) {
    return PREDADORES.filter(x => this.predadorPermitido(x, f, opcoes));
  },
