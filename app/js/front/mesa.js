...
function renderMesa() {
  ...
  <div class="mesa">
    ...
    <div class="mesa-aviso">
      ...
      <span class="so-largo">— respostas pré-escritas, nenhuma IA conectada.
      A interface já está pronta para receber o modelo.</span>
    </div>
...
async function iniciarCampanha(ficha, caminho) {
  const md = await (await fetch(caminho)).text();
  const campanha = Compilador.compilar(md);
  if (campanha.erros.length) {
    toast('Campanha com erros — veja o console.');
    console.warn('Erros de compilação:', campanha.erros);
  }
  iniciarMesa(ficha);
  M.campanha = campanha;
