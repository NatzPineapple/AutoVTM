const Compilador = {
  ...
  lerOpcao(cena, linha, campanha, i) {
    ...
    if (op._lendoRotas && /^-\s*/.test(linha) && linha.includes('+')) {
      ...
      const [a, p] = par.split('+').map(x => this.traco(x));
  ...
  /* ----------------------------------------------------------
     O NOME DO TRAÇO NÃO É O ID DELE  (§72)

     A rota é escrita em português, como o livro e o
     `narracao-ia.md` §4.7 mandam: "Manipulação + Subterfúgio". Mas
     o id interno de Subterfúgio é `labia`, o de Sagacidade é
     `intuicao`, o de Ladroagem é `furto`, o de Erudição é
     `academicos`, o de Ciência é `ciencias` e o de Percepção é
     `consciencia`.

     Antes da §72 o compilador só slugificava, então
     "Subterfúgio" virava `subterfugio` — e `Dados.piscinaDe` lia
     `ficha.habilidades.subterfugio`, que não existe: **zero
     dados, silenciosamente**. A rota parecia válida e valia nada.

     Achado pelo teste da primeira campanha jogável, que confere
     cada rota contra a lista de traços de verdade.

     Aceita as duas grafias: o nome do livro e o id interno. */
  traco(texto) {
    const s = this.slug(texto).replace(/[^a-z_]/g, '');
    if (typeof HABILIDADES === 'undefined' || typeof ATRIBUTOS === 'undefined') return s;
    const listas = [
      ...Object.values(ATRIBUTOS).flatMap(x => x.lista),
      ...Object.values(HABILIDADES).flatMap(x => x.lista)
    ];
    if (listas.some(t => t.id === s)) return s;
    const porNome = listas.find(t => this.slug(t.nome).replace(/[^a-z_]/g, '') === s);
    return porNome ? porNome.id : s;
  },
