...
/* OS CAMPOS PRECISAM DE SEPARADOR.  (§75)

   Era tudo colado: tempo + contador + aleatório, os dois últimos com
   LARGURA VARIÁVEL em base 36. Isso colide sozinho, sem precisar de
   azar no `Math.random`:

     contador 1 (“1”) + aleatório 1295 (“zz”)  →  "1zz"
     contador 71 (“1z”) + aleatório 35 (“z”)   →  "1zz"

   Mesmo milissegundo, mesmo id — e id de sessão repetido é sessão
   sobrescrita em silêncio, que é exatamente o que o item N7 existia
   para impedir. O teste dos mil ids pegava isso de vez em quando, e
   "de vez em quando" é o que faz um teste virar ruído.

   Com o hífen, o contador sozinho já garante unicidade dentro do
   mesmo milissegundo, e o aleatório vira o que devia ter sido desde o
   começo: uma margem contra duas abas na mesma máquina. */
...
function salvarMesa() {
  ...
  try {
    ...
    /* E ESPELHA NO MÓDULO 3.  (§85, item M2)

       Com represa: esta função roda a cada mutação de estado — 45
       chamadas espalhadas pelo front —, e uma requisição por tecla
       digitada no rascunho seria absurdo. A Ponte junta e manda uma vez.

       O `localStorage` acima já gravou: se o módulo estiver fora, ou se
       o espelho falhar, o jogo continua e o F5 continua funcionando.
       Por isso não se espera, e por isso o retorno não muda. */
    Ponte.espelhar(M);
    return true;
  } catch (e) {
