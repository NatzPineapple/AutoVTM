/* ============================================================
   ...
   quanto tempo custa. O juiz é o mesmo validador da produção.
     ...
     node ferramentas/comparador.mjs mistral-nemo:12b granite4.1:8b
     node ferramentas/comparador.mjs --repeticoes 10 mistral-nemo:12b
     node ferramentas/comparador.mjs --camada narrador --repeticoes 10 mistral-nemo:12b
     node ferramentas/comparador.mjs --camada intencao --repeticoes 2 qwen2.5:7b
     node ferramentas/comparador.mjs --camada intencao --sem-contexto qwen2.5:7b
   ============================================================ */

import fs from 'node:fs';
import path from 'node:path';
import { fileURLToPath } from 'node:url';
import { cronicar } from '../modulos/cronista/cronista.mjs';
import { narrar, saudeDoValidador } from '../modulos/cronista/narrador.mjs';
import { extrair, TIPOS } from '../modulos/cronista/intencao.mjs';
...
function argumentos(argv) {
  ...
  if (!opcoes.amostra) {
    opcoes.amostra = opcoes.camada === 'narrador' ? 'modulos/cronista/amostras/turno-camarim.json'
                   : opcoes.camada === 'intencao' ? 'modulos/cronista/amostras/intencoes.json'
                   : 'modulos/cronista/amostras/noite-carnaval.json';
  }
  return opcoes;
}
...
async function principal() {
  ...
  if (!opcoes.modelos.length) {
    ...
    console.log('  node ferramentas/comparador.mjs mistral-nemo:12b granite4.1:8b');
    process.exit(1);
  }
  ...
  if (opcoes.camada === 'intencao') {
    ...
    fs.writeFileSync(path.join(RAIZ, 'modulos/cronista/amostras/ultima-comparacao.json'),
  ...
  const destino = path.join(RAIZ, 'modulos/cronista/amostras/ultima-comparacao.json');
